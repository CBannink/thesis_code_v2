...
def adjusted_plot_quiver(self, pca_embedding, delta_embedding, primary_index, adata, n_neighbors=15, min_dist=0.1, random_state=42, umap_embedding=None, reducer = None):
    _adjusted_plot_quiver(self, pca_embedding= pca_embedding, delta_embedding= delta_embedding, primary_index= primary_index,adata= adata, n_neighbors= n_neighbors, min_dist= min_dist, random_state= random_state, umap_embedding= umap_embedding, reducer= reducer)
...
def _adjusted_plot_quiver(self, pca_embedding, delta_embedding, primary_index,adata,
                         n_neighbors=15, min_dist=0.1, random_state=42,
                         umap_embedding=None, reducer=None,
                         ax=None, scale=CONFIG["scale_simulation"],
                         color=None, s=CONFIG["s_scatter"],
                         show_background=True,
                         args=CONFIG["default_args"]):
    """
    Plot PCA shifts in UMAP space when delta embedding exists for a subset of cells.
    """
    if ax is None:
        ax = plt
    ...
    if self.cell_idx_use is None:
        ix_choice = np.arange(pca_embedding.shape[0])
    else:
        ix_choice = self.cell_idx_use
    self.embedding = umap_embedding
    ...
    # Get the shifted points in PCA space
    shifted_pca = pca_embedding.copy()
    #so primary index en delta embedding is still all but first row is the primary shifted one
    shifted_pca[primary_index] += delta_embedding[0]
    umap_shifted = reducer.transform(shifted_pca)
    ...
    ax.scatter(umap_embedding[ix_choice, 0], umap_embedding[ix_choice, 1],
              c="lightgray", alpha=0.2, edgecolor=(0,0,0,1), s=s, **args)
    ...
    print("starting vis")
    print("ix choice" ,  len(ix_choice))
    # print("shift indices" , len(primary_index))
    print("pca embedding" , pca_embedding.shape)
    print("delta embedding" , delta_embedding.shape)
    print("umap embedding" , umap_embedding.shape)
    print("self embedding" , self.embedding.shape)
    print("umap shifted" , umap_shifted.shape)
    # quiver = umap_embedding

    # ax.quiver(self.embedding[ix_choice, 0], self.embedding[ix_choice, 1],
    #          None,None,
    #           scale=scale, **quiver_kwargs)
    # Plot arrows
    ax.quiver(umap_embedding[ix_choice, 0],
             umap_embedding[ix_choice, 1],
             umap_shifted[primary_index, 0] - umap_embedding[primary_index, 0],
             umap_shifted[primary_index, 1] - umap_embedding[primary_index, 1],
             scale=scale,
             **quiver_kwargs)

    ax.axis("off")
   # sc.pl.umap(self.adata, color='celltype', layer = "normalized_counts")
