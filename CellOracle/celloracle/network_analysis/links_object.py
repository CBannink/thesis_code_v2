...
class Links():
    ...
    def filter_links(self, p=0.001, weight="coef_abs",
        ...
        for i in self.cluster:
            #print("unfiltered: ",self.links_dict[i].shape)
            ...
            #print("filtered: ", self.filtered_links[i].shape)
       ...
       'betweenness_centrality', 'eigenvector_centrality']
        ...
        if hasattr(self, "filtered_links"):
            if isinstance(self.filtered_links, dict):
                ...
                time = datetime.now()
                print("filter current time")
                self.filter_links(p = 0.001, thread_number=2000)
                print("filter done: ", datetime.now()-time)
